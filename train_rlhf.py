...
def maybe_write_comparison_report(output_path: str, results: list[dict[str, object]], args: argparse.Namespace) -> None:
    ...
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"Wrote RLHF comparison report to {path}")


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Train a Robosuite policy with an RLHF pipeline (reward model + PPO)."
    )
    parser.add_argument(
        "--benchmark",
        type=str,
    ...
    parser.add_argument("--value-hidden-sizes", type=parse_hidden_sizes, default=(256, 256))
    parser.add_argument("--reward-hidden-sizes", type=parse_hidden_sizes, default=(256, 256))
    ...
    parser.add_argument("--reward-epochs", type=int, default=8)
    ...
    parser.add_argument("--output", type=str, default="checkpoints/rlhf_policy.pt")
    return parser
...
def main() -> None:
    ...
    try:
        from robosuite_pref_learning import run_rlhf
        ...
        with tee_stdout_stderr(run_args.log_file):
            ...
            if run_args.metrics_output:
                if write_run_metrics_csv(summary, run_args.metrics_output):
                    print(f"Wrote RLHF metrics CSV to {run_args.metrics_output}")
                else:
                    print(f"Skipped RLHF metrics CSV for {run_args.benchmark}/{run_args.env_name}: no history found.")

            if run_args.plot_output:
                if write_run_plot(summary, run_args.plot_output):
                    print(f"Wrote RLHF metrics plot to {run_args.plot_output}")
                else:
                    print(f"Skipped RLHF plot for {run_args.benchmark}/{run_args.env_name}: no history found.")
        results.append(summary)

    if not results:
        raise SystemExit("No RLHF runs completed successfully.")

    if multi_run:
        print("RLHF comparison summary:")
        for result in results:
            print(
